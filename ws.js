(function(ext) {
    ...
    var url = "220.135.86.47";
    var broker_url = "220.135.86.47";
    var username, hashedPass;
    
    /* 連線到系統驗證使用者身分 */
    ext.connectToServer = function(ID, callback){
    	var key = [];
    	key = ID.split(":");
    	if (key.length != 2) {
    	...
    	/* verify input */
    	if(regexpChar.test(username) == false || regexpChar.test(hashedPass) == false) {
    	...
    	/* 用冒號分隔username和md5 hashed password */
    	/* .... */
    	values['username'] = username;
    	values['hashedPass'] = hashedPass;
    ...
    ext.connectSuccess = function(){
    	...
    	if ((isPassAuth && isConnected) == false)
    		return 0;
    	else
    		return 1;
    }
    ...
    var descriptor = {
        blocks: [
            ['w', '我的金鑰 %s 連線到系統', 'connectToServer', '使用者金鑰'],
            ['r', '成功連線', 'connectSuccess'],
            [' ', '訂閱主題 %s', 'subscribeTopic', '主題'],
            [' ', '取消訂閱主題 %s', 'unsubscribeTopic', '主題'],
            ['r', '可否取得 %s 的 %s 裝置的屬性 %s', 'checkUserGetActuator', '提供者', '裝置名稱', '量測數值名稱'],
